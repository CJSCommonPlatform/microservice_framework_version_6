...
public class SnapshotAwareEventStreamManager {
    ...
    @Inject
    DefaultSnapshotService snapshotService;
    ...
    private <T extends Aggregate> void append(final UUID id, final Stream<JsonEnvelope> events, final Optional<Long> versionFrom, final Map<Class<T>, T> aggregatesMap)
    ...
    private JsonEnvelope eventWithVersion(final JsonEnvelope event, final UUID streamId, final Long version) {
        final JsonObjectBuilder stream = createObjectBuilder()
                .add(STREAM_ID, streamId.toString())
                .add(VERSION, version);

        final JsonObjectBuilder metadata = createObjectBuilderWithFilter(event.metadata().asJsonObject(), x -> !STREAM.equals(x))
