...
public class FeedGenerator<E, FE> {
    ...
    private final int PATH_SEGMENT_EVENT_STREAMS = 0;

    private final int PATH_SEGMENT_EVENT_STREAM_ID = 1;
    ...
    private Feed<FE> feedWithLinks(final Link link, final long pageSize, final UriInfo uriInfo,
        ...
        final boolean newEventsAvailable = link != HEAD && recordExists(maxSequenceId + 1, params);
        final boolean olderEventsAvailable = link != LAST && recordExists(minSequenceId - 1, params);
    ...
    private boolean recordExists(final long offset,
                                 final Map<String, Object> params) {
        return repository.recordExists(offset, params);
    }
