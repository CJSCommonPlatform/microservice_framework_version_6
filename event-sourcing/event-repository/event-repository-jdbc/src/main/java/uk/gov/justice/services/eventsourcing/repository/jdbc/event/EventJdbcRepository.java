...
public class EventJdbcRepository extends AbstractJdbcRepository<Event>
    ...
    private static final String SQL_GET_PREVIOUS = "SELECT * FROM event_log t WHERE t.stream_id=? and t.sequence_id  >= ?  ORDER BY t.sequence_id ASC LIMIT ?";
    private static final String SQL_GET_NEXT = "SELECT * FROM event_log t WHERE t.stream_id=? and t.sequence_id  <= ? ORDER BY t.sequence_id DESC LIMIT ?";
    private static final String SQL_GET_LAST = "SELECT * FROM event_log t WHERE t.stream_id=?  ORDER BY t.sequence_id ASC LIMIT ?";
    ...
    public Stream<Event> getFeed(final long offset,
        ...
        try {
            final UUID streamId = UUID.fromString((String) params.get(STREAM_ID));
            if (HEAD.equals(link)) {
                return firstPage(pageSize, streamId);
            }
            if (LAST.equals(link)) {
                return lastPage(pageSize, streamId);
            }
            if (PREVIOUS.equals(link)) {
                return previousPage(offset, pageSize, streamId);
            }
            if (NEXT.equals(link)) {
                return nextPage(offset, pageSize, streamId);
            }
            return Stream.empty();
        } catch (SQLException e) {
    ...
    private Stream<Event> lastPage(final long pageSize, final UUID streamId) throws SQLException {
        final PreparedStatementWrapper ps;
        ps = preparedStatementWrapperOf(SQL_GET_LAST);
        ps.setObject(1, streamId);
        ps.setLong(2, pageSize);
        return streamOf(ps);
    }

    private Stream<Event> nextPage(final long offset, final long pageSize, final UUID streamId)
            throws SQLException {
    ...
    private Stream<Event> previousPage(final long offset, final long pageSize, final UUID streamId)
            throws SQLException {
    ...
    private Stream<Event> firstPage(final long pageSize, final UUID streamId) throws SQLException {
